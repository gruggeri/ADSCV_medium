...
on any possible platform. It is indeed hard, for any passionate in data
...
timeseries plot with R, inspired from the many examples that can be
...
150th case is plotted, for each country. As a second step we will then
...
The data was imported from the [data repository of the John Hopkins
...
by the omnipresent `{tidyverse}`, which allows us to load `{readr}`,
...
dataset by the John Hopkins University, shared on Github.
...
We even tried to be smart here, and select for our pivoting, only the
...
weeks time, given the unfortunately fast pace at which the pandemic
spreads.

In the `country_region` column some countries are reported by
subregions, but we want only one value per day per country. We therefore
...
Now `cases_data` starts looking tidy (one row per country per date and
just one column storing the cases), what a joy to the eyes.
...
calculate the days since the 100th case and after this we can
`ungroup()` the data.
...
daily rise. We then add it to our original `tibble` (which is just
another way to say table in `tidyverse` language). We also create a new
variable `line_type` that we will use to set the line corresponding to
the `33% daily rise` to be a dashed line, in our plot.
...
Now we can do our first attempt at plotting, using the `{ggplot2}`
library. We map the `color` aesthetic to the `country_region` column.
This will apply a different color to each country (in the chosen color
scale). Beware that most categorical color scales do not have more than
12 colors. We also transform the y-axis to a logarithmic scale, using
`scale_y_log10()`. The rest of the code is done to set a color scale and
to set a theme to the plot, as well as all the annotations.
...
`days_since_150`, for each country). Do do so we can use the
...
be therefore ordered by the number of corresponding observations (rows)
in the country. Countries that have more rows (more days since the 100th
case) will come first. We can now plot again the data, using the same
code as before.
...
then order the countries in the reverse order, compared to the one that
we used before. A little hacky trick, indeed.
...
to any chart created using `{ggplot2}`. The only thing we really need to
add is one function: `transition_reveal()`. This will allow us to reveal
...
type of workflow is indeed applicable to anytime we want to create plot,
annotate, and animate a line chart. Hopefully now, when you see a
COVID-19 related chart, you will also be interested on how to replicate
it and why not, animate it.
